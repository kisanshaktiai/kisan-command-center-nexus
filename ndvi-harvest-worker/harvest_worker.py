...
Stores outputs into shared satellite_tiles table (no tenant_id)
"""
...
class NDVIHarvestWorker:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb): await self.http_client.aclose()

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(min=4, max=60))
    async def fetch_tile_scenes(self, tile_id: str, days_back: int = 7) -> List[Dict]:
        """Fetch Sentinel-2 scenes from MPC"""
    ...
    async def process_tile(self, tile_id: str) -> Dict:
        ...
        if not scenes: return {"success": False, "tile_id": tile_id, "error": "No scenes"}
        ...
        storage_path = f"{tile_id}/{datetime.now().date()}/ndvi.tif"
