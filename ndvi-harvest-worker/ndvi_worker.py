...
def get_mgrs_tiles(country_id, limit=50):
    res = supabase.rpc(
        "get_tiles_for_processing", {"p_country_id": country_id, "p_limit": limit}
    ).execute()
    return res.data or []
...
def get_last_date(tile_id, country_id):
    ...
    return date(2020, 1, 1)  # fallback for first-time tiles
...
def upload_record(tile_id, country_id, date_str, ndvi_path, red_url, nir_url, scene):
    ...
    # Upload NDVI raster with overwrite enabled
...
def run():
    ...
    for t in tiles:
        ...
        # ✅ Updated: use .items() instead of deprecated .get_items()
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(["wget", "-q", "-O", red, red_url], check=True)
            subprocess.run(["wget", "-q", "-O", nir, nir_url], check=True)
