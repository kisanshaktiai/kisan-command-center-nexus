...
import { TeamInvitationService, TeamInvitationData } from '@/services/TeamInvitationService';

interface TeamUsersRolesStepProps {
  tenantId: string;
...
export const TeamUsersRolesStep: React.FC<TeamUsersRolesStepProps> = ({
  tenantId,
  onComplete,
  data,
  onDataChange
  ...
  const loadTenantData = async () => {
    try {
      // Note: This would need to be implemented based on your tenant data fetching logic
      setTenantData({ name: 'Your Organization' });
    } catch (error) {
  ...
  const loadInvitations = async () => {
    try {
      setIsLoading(true);
      const invitationsList = await TeamInvitationService.getTeamInvitations(tenantId);
  ...
  const handleInviteUser = async () => {
    ...
    try {
      ...
      const response = await TeamInvitationService.sendTeamInvite({
        tenantId,
        email: newUser.email,
      ...
      if (!response.success) {
        throw new Error(response.error || 'Failed to send team invitation');
      }

      showSuccess(`Team invitation sent to ${newUser.email}`);
      ...
      showError(error.message || 'Failed to send team invitation');
    } finally {
      setIsSending(null);
    }
  };

  const handleResendInvitation = async (invitation: TeamInvitationData) => {
    try {
      ...
      const response = await TeamInvitationService.sendTeamInvite({
        ...
        tenantName: invitation.tenant_name || 'Your Organization',
        inviterName: invitation.inviter_name || 'Team Admin'
      });

      if (!response.success) {
        throw new Error(response.error || 'Failed to resend team invitation');
      }

      showSuccess('Team invitation resent successfully');
      await loadInvitations();
    } catch (error: any) {
      console.error('Error resending team invitation:', error);
      showError(error.message || 'Failed to resend team invitation');
    } finally {
      setIsSending(null);
    }
  };
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Manage team invitations and role assignments
              </CardDescription>
            </div>
