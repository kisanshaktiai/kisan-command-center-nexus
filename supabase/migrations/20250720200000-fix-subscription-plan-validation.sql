...
-- Fix subscription plan validation in create_tenant_with_validation function
...
BEGIN
  ...
  -- Validate subscription plan (updated to use correct enum values from database)
  IF p_subscription_plan NOT IN ('Kisan_Basic', 'Shakti_Growth', 'AI_Enterprise', 'custom') THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Invalid subscription plan. Must be one of: Kisan_Basic, Shakti_Growth, AI_Enterprise, custom',
      'code', 'VALIDATION_ERROR'
    );
  END IF;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        -- Ignore if tenant_branding table doesn't exist
        NULL;
    END;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        -- Ignore if tenant_features table doesn't exist
        NULL;
    END;
