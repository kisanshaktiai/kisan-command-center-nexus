-- Update subscription plan enum to new naming convention
...
ALTER TYPE public.subscription_plan ADD VALUE IF NOT EXISTS 'kisan_starter';
ALTER TYPE public.subscription_plan ADD VALUE IF NOT EXISTS 'shakti_growth';
ALTER TYPE public.subscription_plan ADD VALUE IF NOT EXISTS 'ai_enterprise';

-- Step 2: Update existing data to use new values
UPDATE public.tenants 
SET subscription_plan = CASE
  WHEN subscription_plan = 'kisan' THEN 'kisan_starter'::subscription_plan
  WHEN subscription_plan = 'shakti' THEN 'shakti_growth'::subscription_plan  
  WHEN subscription_plan = 'ai' THEN 'ai_enterprise'::subscription_plan
  ELSE subscription_plan
END;

-- Step 3: Update the create_tenant_with_validation function to use new enum values
