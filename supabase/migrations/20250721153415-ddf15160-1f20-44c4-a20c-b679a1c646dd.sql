...
-- Step 1.1: Fix ENUM Casting in create_tenant_with_validation function
-- Drop existing function to recreate with proper ENUM handling
DROP FUNCTION IF EXISTS public.create_tenant_with_validation;

-- Recreate the function with proper ENUM casting and error handling
...
BEGIN
  ...
  -- Safely cast and validate tenant type
  BEGIN
    v_tenant_type := p_type::tenant_type;
  EXCEPTION
    WHEN invalid_text_representation THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Invalid tenant type: ' || p_type || '. Must be one of: agri_company, dealer, ngo, government, university, sugar_factory, cooperative, insurance',
        'code', 'VALIDATION_ERROR'
      );
  END;
  
  -- Safely cast and validate tenant status
  BEGIN
    v_tenant_status := p_status::tenant_status;
  EXCEPTION
    WHEN invalid_text_representation THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Invalid status: ' || p_status || '. Must be one of: trial, active, suspended, cancelled',
        'code', 'VALIDATION_ERROR'
      );
  END;
  
  -- Safely cast and validate subscription plan
  BEGIN
    v_subscription_plan := p_subscription_plan::subscription_plan;
  EXCEPTION
    WHEN invalid_text_representation THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Invalid subscription plan: ' || p_subscription_plan || '. Must be one of: Kisan_Basic, Shakti_Growth, AI_Enterprise, custom',
        'code', 'VALIDATION_ERROR'
      );
  END;
  ...
  RAISE NOTICE 'Inserting tenant with plan: % and limits: %', v_subscription_plan, v_plan_limits;
  
  -- Begin transaction for atomic operations
  BEGIN
    -- Insert tenant with properly cast ENUMs
    INSERT INTO tenants (
      name,
      slug,
      type,
    ...
    -- Create default tenant branding (only if tenant_branding table exists)
    BEGIN
    ...
    -- Create tenant features (only if tenant_features table exists)
    BEGIN
