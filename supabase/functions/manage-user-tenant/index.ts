...
serve(async (req) => {
  ...
  try {
    ...
    // Check if this is a service role call (bypass auth for internal operations)
    const apiKey = req.headers.get('apikey');
    const serviceRoleKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');
    const isServiceRoleCall = apiKey === serviceRoleKey;

    let user: any = null;
    let bypassAuth = false;

    if (isServiceRoleCall) {
      console.log(`[${securityContext.requestId}] Service role key detected - allowing internal operation`);
      bypassAuth = true;
    } else {
    ...
    // Check if user is admin (skip if service role call)
    let hasPermission = bypassAuth;
    let adminRole = bypassAuth ? 'service_role' : '';
    let adminUser: any = null;

    if (!bypassAuth && user) {
      const { data: fetchedAdminUser, error: adminCheckError } = await supabase
        .from('admin_users')
    ...
    if (req.method === 'POST') {
      ...
      console.log(`[${securityContext.requestId}] Verifying user exists in auth.users:`, requestBody.user_id);
      try {
        const { data: authUser, error: authError } = await supabase.auth.admin.getUserById(requestBody.user_id);
        
        if (authError || !authUser.user) {
          console.error(`[${securityContext.requestId}] User not found in auth.users:`, authError);
          return new Response(
            JSON.stringify({ 
              success: false, 
              error: 'User does not exist in authentication system',
              code: 'USER_NOT_FOUND',
              request_id: securityContext.requestId
            }),
            { 
        ...
        console.log(`[${securityContext.requestId}] User verified in auth.users:`, authUser.user.email);
      } catch (authCheckError) {
        console.error(`[${securityContext.requestId}] Error checking auth.users:`, authCheckError);
        return new Response(
          JSON.stringify({ 
            success: false, 
      ...
      // Additional security check: tenant admins can only manage their own tenants (skip for service role)
      if (adminRole === 'tenant_admin' && !bypassAuth) {
      ...
      const { data: result, error: dbError } = await supabase.rpc(
        'manage_user_tenant_relationship',
        {
          ...
          p_metadata: {
            ...
            managed_by: user?.id || 'service_role',
